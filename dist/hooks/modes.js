...
import { startMode, endMode, readState, transition, updateData, isLastPhase, } from '../core/state.js';
...
export function createPipelineMode(config = {}) {
    ...
    const hooks = [
        // Inject phase-specific context on each prompt
        {
            ...
            handle: () => {
                const state = readState();
                if (state.mode !== MODE.pipeline || !state.phase)
                    return {};

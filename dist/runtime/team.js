...
function buildShellCommand(task, resultDir) {
...
// ---------------------------------------------------------------------------
function spawnWorkers(batch, sessionName) {
    mkdirSync(PATHS.teamResults, { recursive: true });
    const first = batch[0];
    tmux('new-session', '-d', '-s', sessionName, '-x', '200', '-y', '50', buildShellCommand(first, PATHS.teamResults));
    for (let i = 1; i < batch.length; i++) {
        const task = batch[i];
        tmux('split-window', '-t', sessionName, '-h', buildShellCommand(task, PATHS.teamResults));
        tmux('select-layout', '-t', sessionName, 'tiled');
    }
}
function readResult(taskId) {
 ...
 */
export function runTeam(tasks, config = {}) {
    if (!tmuxExists()) {
...
export function getTeamStatus() {
    if (!existsSync(PATHS.teamSession))
        return [];
    const session = JSON.parse(readFileSync(PATHS.teamSession, 'utf-8'));

...
export function createStopGuard(
...
export function createPromptEnhancer(
  formatContext?: (mode: string, phase: string | null, data: Record<string, unknown>) => string,
): HookHandler {
  const defaultFormat = (mode: string, phase: string | null, data: Record<string, unknown>) => {
...
export function createSessionResume(): HookHandler {
  return {
    event: 'SessionStart',
    handle: () => {
      const state = readState();
      ...
      return {
        additionalContext:
          `[Resuming mode: ${state.mode}${phaseInfo}]`
          + remaining

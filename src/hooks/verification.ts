 ...
 */
export function createVerificationHook(config: VerificationConfig = {}): HookHandler {
  ...
  return {
    ...
    handle: (input) => {
      ...
      if (output.includes('Background task launched')) return {};

      let context = message;
      if (showDiff) context += getGitDiffSummary();
